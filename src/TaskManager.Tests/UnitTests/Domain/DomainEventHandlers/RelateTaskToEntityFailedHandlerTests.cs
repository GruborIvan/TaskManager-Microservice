...
{
    public class RelateTaskToEntityFailedHandlerTests
    {
        ...
        {
            //Arrange
            _mockEventNotificationService.Setup(s => s.SendAsync(It.IsAny<RelateTaskToEntityFailedEvent>(), It.IsAny<string>()));
            _mockEventStreamingService.Setup(s => s.SendAsync(It.IsAny<RelateTaskToEntityFailedEvent>(), It.IsAny<CancellationToken>()));

            RelateTaskToEntityFailed notification = new RelateTaskToEntityFailed(Guid.NewGuid(), new ErrorData("error message", "errorCode"));

            RelateTaskToEntityFailedHandler handler = new RelateTaskToEntityFailedHandler(_mockEventNotificationService.Object, _mockEventStreamingService.Object);
