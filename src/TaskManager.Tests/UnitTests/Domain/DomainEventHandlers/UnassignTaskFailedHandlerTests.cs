...
{
    public class UnassignTaskFailedHandlerTests
    {
        ...
        {
            //Arrange
            _mockEventNotificationService.Setup(s => s.SendAsync(It.IsAny<UnassignTaskFailedEvent>(), It.IsAny<string>()));
            _mockEventStreamingService.Setup(s => s.SendAsync(It.IsAny<UnassignTaskFailedEvent>(), It.IsAny<CancellationToken>()));

            UnassignTaskFailed notification = new UnassignTaskFailed(Guid.NewGuid(), new ErrorData("error message", "errorCode"));

            UnassignTaskFailedHandler handler = new UnassignTaskFailedHandler(_mockEventNotificationService.Object, _mockEventStreamingService.Object);
        ...
        public async System.Threading.Tasks.Task Invalid_Notification_EventNotificationService_Throws_Exception()
        {
            ...
            UnassignTaskFailed notification = new UnassignTaskFailed(Guid.Empty, new ErrorData("error message", "errorCode"));
