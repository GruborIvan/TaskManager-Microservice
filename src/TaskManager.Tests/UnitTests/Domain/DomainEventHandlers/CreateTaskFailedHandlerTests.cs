...
{
    public class CreateTaskFailedHandlerTests
    {
        ...
        {
            //Arrange
            _mockEventNotificationService.Setup(s => s.SendAsync(It.IsAny<CreateTaskFailedEvent>(), It.IsAny<string>()));
            _mockEventStreamingService.Setup(s => s.SendAsync(It.IsAny<CreateTaskFailedEvent>(), It.IsAny<CancellationToken>()));

            CreateTaskFailed notification = new CreateTaskFailed(Guid.NewGuid(), new ErrorData("error message", "errorCode"));

            CreateTaskFailedHandler handler = new CreateTaskFailedHandler(_mockEventNotificationService.Object, _mockEventStreamingService.Object);
        ...
        public async System.Threading.Tasks.Task Invalid_Notification_EventNotificationService_Throws_Exception()
        {
            ...
            CreateTaskFailed notification = new CreateTaskFailed(Guid.Empty, new ErrorData("error message", "errorCode"));
