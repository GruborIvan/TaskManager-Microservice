...
{
    public class UpdateDataFailedHandlerTests
    {
        ...
        {
            //Arrange
            _mockEventNotificationService.Setup(s => s.SendAsync(It.IsAny<UpdateTaskDataFailedEvent>(), It.IsAny<string>()));
            _mockEventStreamingService.Setup(s => s.SendAsync(It.IsAny<UpdateTaskDataFailedEvent>(), It.IsAny<CancellationToken>()));

            UpdateDataFailed notification = new UpdateDataFailed(Guid.NewGuid(), new ErrorData("error message", "errorCode"));

            UpdateDataFailedHandler handler = new UpdateDataFailedHandler(_mockEventNotificationService.Object, _mockEventStreamingService.Object);
