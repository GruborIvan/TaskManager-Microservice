...
{
    public class StoreCommentFailedHandlerTests
    {
        ...
        {
            //Arrange
            _mockEventNotificationService.Setup(s => s.SendAsync(It.IsAny<StoreCommentFailedEvent>(), It.IsAny<string>()));
            _mockEventStreamingService.Setup(s => s.SendAsync(It.IsAny<StoreCommentFailedEvent>(), It.IsAny<CancellationToken>()));

            StoreCommentFailed notification = new StoreCommentFailed(Guid.NewGuid(), new ErrorData("error message", "errorCode"));

            StoreCommentFailedHandler handler = new StoreCommentFailedHandler(_mockEventNotificationService.Object, _mockEventStreamingService.Object);
        ...
        public async System.Threading.Tasks.Task Invalid_Notification_EventNotificationService_Throws_Exception()
        {
            ...
            StoreCommentFailed notification = new StoreCommentFailed(Guid.Empty, new ErrorData("error message", "errorCode"));
